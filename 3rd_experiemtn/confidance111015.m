...
x_max =   max(updated_data(:,17)); %6.488201490499051e+05; %
x_min = min(updated_data(:,17));


y_max= max(updated_data(:,18)); %6.821629319075700e+06; %
y_min=min(updated_data(:,18));

%x_max-x_min; y_max-y_min;

n= 100; % amount of grid along xy coordinate
...
%% test data dividation

load DATA_RFFP_SEPT14_LTE_WLAN_7
UE_data = DATA_RFFP_SEPT14_LTE_WLAN_7;
UE_non_zero_xy= find(UE_data(:,17)~= 0 & UE_data(:,18)~= 0 & UE_data(:,7)~= 0  & UE_data(:,8)~= 0 & UE_data(:,9)~= 0 & UE_data(:,3)>=70060 & UE_data(:,3)<=70084 & UE_data(:,17)<= 6.488201490499051e+05 & UE_data(:,18)<= 6.821629319075700e+06   ); %
UE_updated_data = UE_data(UE_non_zero_xy,:);

n=100;

step = floor(size(UE_updated_data,1)/n);
...
for jj = 1:n
    ...
    if jj==1
        UE_block{jj}=  UE_updated_data(l:u,:);
        ...
        for mm=1:length(Training_signature)
            ...
            if size(Training_signature{mm},1)~=0
                ...
                for kk=1:size(checking_index,1)
                    ...
                    if checking_index(kk,1)>=3   % variable
...
%experiemnt
% number = 180;
% x=confidance_sorted_modle{number}(end,15);
% y=confidance_sorted_modle{number}(end,16);
% r = 50;
% point = confidance_sorted_modle{number}(1:end-1,15:16);
% draw(x,y,r,point)
% 
% p= confidance_sorted_modle{number}(1:end-1,15);
% dlmwrite('x.dat', p)
% 
% dlmwrite('alil.dat', point)
% 
% q= confidance_sorted_modle{number}(1:end-1,16);
% dlmwrite('y.dat', q)
% 
% 
% number = 10;
% plot(confidance_sorted_modle{number}(1:end-1,15), confidance_sorted_modle{number}(1:end-1,16),'*'); 
% hold on
% plot(confidance_sorted_modle{number}(end,15), confidance_sorted_modle{number}(end,16),'*');
% %experiment
...
for ii = 1:length(confidance_sorted_modle)
    ...
    if size(confidance_sorted_modle{ii},1)~=0  %%I change it
        ...
        low=vertcat(confidance_sorted_modle{ii}(1,:),lowest_dist);
